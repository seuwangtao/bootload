...
// DDR size print added.
// ********************************************************************************
//#include "pilot_II.h"
#define HW_ENV  1

#include  "./include/pilot_types.h"
#include  "./include/ddr.h"
#include  "./include/xdma.h"
...
//Our ddr init automatically takes care of all the MEMORY TYPES,so do not change any of the following defines
#if defined TYPE1
#elif defined TYPE2
#elif defined TYPE3
#elif defined TYPE4
...
{
    // make sure the compiler does not optimize away
    // this code.
  tVU32 i=0;
  for(i=0;i<count;i++)
    ;
...
#if defined ECC
    nc_printf("DDR3 Memory size 256MB\n");
#else
    nc_printf("DDR3 Memory size 512MB\n");
#endif
#endif
#endif
  }
...
#if defined (ENABLE_NCPRINTF_FEW)
#if defined ECC
    nc_printf("DDR3 Memory size 128MB\n");
#else
    nc_printf("DDR3 Memory size 256MB\n");
#endif
#endif
...
#if defined (ENABLE_NCPRINTF_FEW)
#if defined ECC
    nc_printf("DDR3 Memory size 64MB\n");
#else
    nc_printf("DDR3 Memory size 128MB\n");
#endif
#endif
...
tU32 ddrinit_asic(void){
  ...
  *(tPVU32)(MC_REG_ADDRESS04) = temp;
  ...
  /*
   * Make 3 tries to probe the size of dram. If two consectutive checks match
   * go with that value. If after three tries we don't get a match, just
   * go with the last checked value.
   */

  for (i = 0; i < 3; i++) {
      ddr_calc_type = ddrsize_calc();
      int recheck_ddr_calc_type = ddrsize_calc();
      if (ddr_calc_type == recheck_ddr_calc_type) {
          break;
      }
  }
  ...
  *(tPVU32)(MC_REG_ADDRESS10) = temp;
  ...
  if (ddr_type == 2) {
    ...
    *(tPVU32)(MC_REG_ADDRESS00) = temp;
    ...
    tU32 i=0;
    for(i=0;i<31;i++) {
        __asm__ __volatile__ ("mov %0, #0" : : "r" (temp));
        __asm__ __volatile__ ("mcr p15, 0, %0, c7, c5, 0" : : "r" (temp));
        DELAY(100*100);
    }
  }
